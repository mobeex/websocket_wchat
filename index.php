	...
	foreach($classes AS $classe){
		if (strlen($classe) >= 2) {
			switch (substr($classe, 0, 1)) {
				case 'P': $provedorID = substr($classe, 1, 15); break;
				case 'C': $chatID = substr($classe, 1, 15); break;
				case 'S': $sessao = substr($classe, 1, 15); break;
				case 'F': $flagID = substr($classe, 1, 15); break;
				case 'U': $userID = substr($classe, 1, 15); break;
				case 'G': $deptoID = substr($classe, 1, 15); break;
				case 'R': $registro = substr($classe, 1, 15); break;
				default: $elemento = $classe;
			}
		}
	}
	$url = 'https://www.megalochat.com/sandbox/apis/consumida/publica/index.php?' . 
